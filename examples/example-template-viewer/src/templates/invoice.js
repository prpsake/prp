import { html, modelQR } from "@prpsake/template-viewer"
...
export const model = {
  ...
  emailSubject: ({ number, subject }) => encodeURIComponent(`Rechnung ${number} | ${subject}`),
  qr: data => modelQR({
...
export const content = ({ data }) => html`
  ...
  <footer class="bottom-left -mt-[1.5mm] font-mono text-template-fg">
    <div class="flex text-2xs">
      <div class="flex-none w-page-left">
        <p class="font-extralight">Alle Preise in Schweizer Franken.</p>
      ...
      <div class="flex-auto">
        ...
        <p class="font-extralight">${data.creditor.street} ${data.creditor.streetNumber}</p>
        <p class="font-extralight">${data.creditor.postalCode} ${data.creditor.locality}</p>
        <br>
        <dl>
          <div class="flex">
            <dt class="font-medium italic">e-mail</dt>&nbsp;
            <dd class="font-extralight">
              <a href="mailto:${data.creditor.email}?subject=${data.emailSubject}">${data.creditor.email}</a>
            </dd>
          </div>
          ${data.creditor.threema && html`
            <div class="flex">
  ...
  <main class="flex font-mono text-template-fg">
    <section class="flex-auto order-2">
      <h1 class="font-extrabold text-4xl pb-[1.55rem]">Rechnung</h1>
      <dl class="table text-sm">
        ...
        <div class="table-row">
          <dt class="table-cell pr-8 pb-4 font-medium italic">Fällig am</dt>
          <dd class="table-cell pb-4 font-extralight">${data.due}, netto</dd>
        </div>
        <div class="table-row">
          <dt class="table-cell pr-8 pb-4 font-medium italic">Betrag</dt>
          <dd class="table-cell pb-4 font-extralight">${data.currency} ${data.amount}</dd>
        </div>
      </dl>
    </section>
    <section class="flex-none w-page-left pt-[1.55rem] pr-[20mm] order-1 text-3xs">
      ...
      <ul>
        ${data.items.map((item, i) => html`
          <li class="relative pb-8 break-inside-avoid">
            <div class="absolute w-[6px] h-[6px] bg-template-fg top-[2px] -left-[10mm]"></div>
            <div class="flex justify-between pb-2">
              <span class="w-1/3 font-medium italic">${i + 1}</span>
              <span class="w-1/6 text-center pl-2 font-extralight">${item.quantity}</span>
              <span class="w-1/6 text-center pl-3 font-extralight">${item.unit}</span>
              <span class="w-1/6 text-right pr-1 font-extralight">${item.price}</span>
              <span class="w-1/6 text-right font-extralight">${item.total}</span>
            </div>
            <p class="w-5/6 font-extralight">${item.description}</p>
            ${item.issues && item.issues.length > 0 && html`
              <p class="w-full pt-2">
      ...
      <footer class="relative flex justify-between">
        ...
        <span class="w-1/6 text-right font-extralight">${data.currency} ${data.amount}</span>
      </footer>
    </section>
    <section class="page-payment-methods">
      <a-qr-bill data=${data.qr}></a-qr-bill>
    </section>
  </main>
`
