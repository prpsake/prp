...
export const model = {
  ...
  emailSubject: ({ number, subject }) => encodeURIComponent(`Kostenschätzung ${number} | ${subject}`)
}
...
export const content = ({ data }) => html`
  ...
  <footer class="bottom-left -mt-[1.5mm] font-mono text-template-fg">
    <div class="flex text-2xs">
      <div class="flex-none w-page-left">
        <p class="font-extralight">Alle Preise in Schweizer Franken und unverbindlich.</p>
      ...
      <div class="flex-auto">
        ...
        <p class="font-extralight">${data.creditor.postOfficeBox || (data.creditor.street + " " + data.creditor.streetNumber)}</p>
  ...
  <header class="flex font-mono text-template-fg">
    ...
    <div class="flex-auto pt-20 pb-28 font-extralight text-sm">
      ...
      <p>${data.debtor.postOfficeBox || (data.debtor.street + " " + data.debtor.streetNumber)}</p>
      <p>${data.debtor.postalCode} ${data.debtor.locality}</p>
    </div>
  </header>
  <main class="flex font-mono text-template-fg">
    <section class="flex-auto order-2">
      <h1 class="font-extrabold text-3xl pt-[0.3rem] pb-[1.5rem]">Kostenschätzung</h1>
      ...
      <ul>
        ${data.items.map((item, i) => html`
          <li class="relative pb-8 break-inside-avoid">
            ...
            ${Boolean(item.issues?.length) && html`
              <p class="w-full pt-2">
