...
*/
let removeWhitespace: string => string = x =>
...
*/
let reverseStr: string => string = x =>
  Js.String2.split(x, "")->Js.Array2.reverseInPlace->Js.Array2.joinWith("")
...
*/
let blockStr: (int, string) => string = (n, x) =>
  switch Js.Types.classify(x) {
  | JSString(x) =>
    let pat = Js.Re.fromStringWithFlags("\\S{" ++ Belt.Int.toString(n) ++ "}", ~flags="g")
    removeWhitespace(x)->Js.String2.replaceByRe(pat, "$& ")->Js.String2.trim
  | _ => ""
  }
...
let blockStrRight3: string => string = x => reverseStr(x)->blockStr3->reverseStr
...
*/
let referenceBlockStr: string => string = x =>
  switch Js.Types.classify(x) {
  | JSString(x) =>
...
*/
let moneyFromNumberStr: (int, string) => string = (n, x) =>
  switch Js.Types.classify(x) {
  | JSString(x) =>
    removeWhitespace(x)
    ->Js.Float.fromString
